...
def make_large_uniform_history(n=1000):
    """创建大样本均匀分布数据."""
    random.seed(42)
    records = []
...
def make_large_cold_history(n=1000, cold_digit=0):
    """创建大样本冷号数据."""
    random.seed(42)
    records = []
...
def test_large_sample_uniform():
    """测试大样本均匀数据."""
    print("=" * 80)
    print("测试大样本均匀数据 (n=1000)")
    print("=" * 80)
    
    records = make_large_uniform_history(1000)
...
def test_large_sample_cold():
    """测试大样本冷号数据."""
    print("\n" + "=" * 80)
    print("测试大样本冷号数据 (n=1000)")
    print("=" * 80)
    
    records = make_large_cold_history(1000, cold_digit=0)
    strategy = FC3DMissingNumberStrategy()
...
def test_false_positive_rate_large():
    """测试大样本下的假阳性率."""
    print("\n" + "=" * 80)
    print("测试大样本下的假阳性率")
    print("=" * 80)
...
def test_statistical_power_large():
    """测试大样本下的统计功效."""
    print("\n" + "=" * 80)
    print("测试大样本下的统计功效")
    print("=" * 80)
    ...
    for i in range(total_tests):
        ...
        # 检查是否正确检测到显著冷号
        if "统计显著偏冷号码" in basis and "0" in basis:
            true_positives += 1
...
if __name__ == "__main__":
    test_large_sample_uniform()
    test_large_sample_cold()
    test_false_positive_rate_large()
    test_statistical_power_large()
