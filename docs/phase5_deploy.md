...
- **角色**：`admin` / `user`。`User.role` 默认 `user`。
- **初始化引导**：系统无任何用户时，首个成功注册的用户自动成为 `admin`；
  后续注册均为普通用户。生产部署后请立即注册管理员账号。
- **管理后台**：`/admin/*` 接口要求 `admin` 角色（非管理员返回 403）。前端在
  `/me` 返回 `role==admin` 时显示「管理」标签，可查看统计、调整用户角色、删除用户
  （禁止管理员操作自身）。
- **既有库迁移**：`init_db()` 会在启动时为已存在的 `users` 表补齐 `role` 列
  （默认值 `user`），旧用户不会自动获得管理员权限；如需提升，请用现有管理员在
  后台调整或手动更新数据库。
