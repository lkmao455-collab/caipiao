...
def make_realistic_history_v2(n=100):
    ...
    for i in range(n):
        ...
        for p in range(3):
            if p == 0:
                # 位置0：数字5出现次数特别多，数字0出现次数特别少
                if random.random() < 0.3:  # 30%概率选数字5
                    digit = 5
                elif random.random() < 0.1:  # 10%概率选数字0
                    digit = 0
                else:
                    digit = random.randint(0, 9)
            elif p == 1:
                # 位置1：数字7出现次数特别多
                if random.random() < 0.25:  # 25%概率选数字7
                    digit = 7
                else:
                    digit = random.randint(0, 9)
            else:
                # 位置2：数字9出现次数特别多
                if random.random() < 0.2:  # 20%概率选数字9
                    digit = 9
                else:
                    digit = random.randint(0, 9)
            pos.append(digit)
...
def test_statistical_significance_v2():
    """测试统计显著性检验."""
    print("=" * 80)
    print("测试统计显著性检验 (v2)")
    print("=" * 80)
    
    records = make_realistic_history_v2(100)
    ...
    for i, options in enumerate(test_cases):
        ...
        # 检查是否偏向热号（5, 7, 9）
        hot_digits = [5, 7, 9]  # 我们故意增加的数字
        hot_freq = sum(digit_counts.get(d, 0) for d in hot_digits) / total
        print(f"  热号(5,7,9)总频率: {hot_freq:.3f}")
...
def test_probability_distribution_v2():
    """测试概率分布改进."""
    print("\n" + "=" * 80)
    print("测试概率分布改进 (v2)")
    print("=" * 80)
...
def test_chi_square_guard_v2():
    """测试χ²均匀性检验守卫."""
    print("\n" + "=" * 80)
    print("测试χ²均匀性检验守卫 (v2)")
    print("=" * 80)
...
def test_parameter_sensitivity_v2():
    """测试参数敏感性."""
    print("\n" + "=" * 80)
    print("测试参数敏感性 (v2)")
    print("=" * 80)
    ...
    # 测试不同z_threshold
    print("\nz_threshold参数对结果的影响:")
    for z_thresh in [150, 196, 258, 300]:
        options = {
            "history": records,
            "lookback": 100,
        ...
        print(f"  z阈值={z_thresh/100:.2f}: 热号(5,7,9)频率={hot_freq:.3f}")


def test_basis_explanation_v2():
    """测试策略说明文本."""
    print("\n" + "=" * 80)
    print("测试策略说明文本 (v2)")
    print("=" * 80)
...
def test_comparison_with_old_strategy():
    ...
    # 新策略
    new_strategy = FC3DMissingNumberStrategy()
    new_tickets = new_strategy.generate(count=100, options={
        "history": records,
        "lookback": 100,
        "z_threshold": 196,
        "temperature": 5,
    ...
    for ticket in new_tickets:
        new_digits.extend(ticket.groups["pos"])
    
    new_digit_counts = Counter(new_digits)
    new_total = len(new_digits)
    
    print("\n新策略（基于统计显著性）:")
    print(f"  生成号码数量: {len(new_tickets)}")
    print(f"  数字分布: {dict(sorted(new_digit_counts.items()))}")
    
    # 计算每个数字的频率
    for d in range(10):
        freq = new_digit_counts.get(d, 0) / new_total
        print(f"    数字{d}: {freq:.3f} ({new_digit_counts.get(d, 0)}/{new_total})")
    
    # 计算分布熵
    def entropy(counts, total):
        probs = [counts.get(d, 0) / total for d in range(10)]
        return -sum(p * math.log2(p) for p in probs if p > 0)
    
    max_entropy = math.log2(10)
    new_entropy = entropy(new_digit_counts, new_total)
    print(f"  分布熵: {new_entropy:.3f} / {max_entropy:.3f} ({new_entropy/max_entropy*100:.1f}%)")
    
    # 计算热号频率
    hot_digits = [5, 7, 9]
    hot_freq = sum(new_digit_counts.get(d, 0) for d in hot_digits) / new_total
    print(f"  热号(5,7,9)频率: {hot_freq:.3f}")


if __name__ == "__main__":
    test_statistical_significance_v2()
    test_probability_distribution_v2()
    test_chi_square_guard_v2()
    test_parameter_sensitivity_v2()
    test_basis_explanation_v2()
