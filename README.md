# 彩票号码生成器（双色球 / 福彩3D / 七乐彩 / 快乐8）

基于 Python + PySide6 的多彩种彩票号码自动生成软件，采用模块化、插件化架构，具备良好的可扩展性。核心支持双色球、福彩3D、七乐彩、快乐8，各彩种共享统一的生成策略与回测框架。

## 功能特性

- **多种生成策略**（双色球 18 种，福彩3D 18 种，七乐彩/快乐8 10 种）
  - 完全随机、奇偶均衡、冷热号分析、排除/必含
  - 智能冷热号、遗漏号追踪、历史均衡
  - **XGBoost / LightGBM / CatBoost 智能分析**
  - **随机森林分析**（集成学习，鲁棒性强）
  - **贝叶斯推断**（融合历史先验与近期观测）
  - **马尔可夫链分析**（状态转移矩阵建模）
  - **趋势分析**（滑动窗口识别上升/下降趋势）
  ...
  - **集成投票分析**（多模型软投票融合）
  - **相关性挖掘**（号码共现与条件概率）
  - **Transformer 时序预测**（自注意力机制捕捉长期依赖）
  - 可通过插件扩展更多策略

- **号码展示**
  - 红球/蓝球/福彩3D/七乐彩/快乐8 图形化展示
  - 紧凑文本格式，支持一键复制
...
- **界面与主题**
  - 深色/浅色主题
  - 全局字体统一使用 **pt** 单位，避免启动时出现 `QFont::setPointSize: Point size <= 0` 警告
  - **策略下拉选择框**：支持 18 种策略，切换策略时自动显示对应参数面板
