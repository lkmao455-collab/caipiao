...
class LightGBMStrategy(GenerationStrategy):
    ...
    def generate(
        ...
        # LightGBM 作为科学实验应保持确定性：使用固定随机种子，
        # 在相同历史数据和参数下每次输出一致。
        FIXED_SEED = 42
        ...
        model_path = find_current_model(
            records, lookback, prefix=MODEL_PREFIX
        ) or new_model_path(lookback, prefix=MODEL_PREFIX)
        ...
        basis = (
            f"LightGBM 智能分析策略：基于全部 {len(records)} 期历史数据训练模型，"
