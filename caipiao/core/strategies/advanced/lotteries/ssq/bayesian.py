...
class SSQBayesianStrategy(SSQAdvancedStrategy):
    """基于贝叶斯推断的号码生成策略。"""
    ...
    def _compute_probabilities(
        ...
        proba = self._bayesian_inference(
            analyzer, "red", 33, lookback, prior_weight, alpha
        )

        basis = (
            f"贝叶斯推断（双色球）：先验权重 {int(prior_weight * 100)}%，"
            f"观测窗口 {lookback} 期。注意：历史统计规律不能预测独立随机开奖，本策略仅作为号码筛选参考。"
        )
        return proba, basis
