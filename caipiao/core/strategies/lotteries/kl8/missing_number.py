...
class KL8MissingNumberStrategy(GenerationStrategy):
    ...
    def generate(
        ...
        # 快乐8：80个号码，每期开20个，p=20/80=0.25
        p = 0.25
        expected = (1 - p) / p  # 3
        sigma = math.sqrt(1 - p) / p  # ≈3.46
        ...
        basis += (
            f"数学说明：遗漏值服从几何分布Geom(p=0.25)，期望={expected}期(σ≈{sigma:.1f})。"
