...
class PL3MissingNumberStrategy(GenerationStrategy):
    ...
    def generate(
        ...
        for n, missing_periods in missing:
            z_score = (missing_periods - expected) / sigma
        ...
        if significant_cold:
            cold_desc = [f"{n}(z={z})" for n, _, z in significant_cold[:5]]
            basis += f"统计显著偏冷号码(z>{z_threshold}): {', '.join(cold_desc)}。"
        else:
            basis += f"无统计显著偏冷号码(z>{z_threshold})，使用高遗漏号码候选池。"

        basis += (
            f"数学说明：遗漏值服从几何分布Geom(p=0.1)，期望={expected}期(σ≈{sigma:.1f})。"
